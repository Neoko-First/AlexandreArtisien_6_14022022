...
// système de fichiers, donne accès aux fonctions qui nous permettent de modifier le système de fichiers, y compris aux fonctions permettant de supprimer les fichiers
const fs = require("fs");
...
exports.modifySauce = (req, res, next) => {
  ...
  const sauceObject = req.file
    ...
    : // sinon, envoi le corps de la requête sans nouvelle image
      { ...req.body };
...
exports.likeSauce = (req, res, next) => {
  ...
  Sauce.findOne({ _id: idLikeSauce })
    .then((sauce) => {
      ...
      if (stateLike == 1) {
        Sauce.updateOne(
          ...
          .catch((error) =>
            res.status(400).json({ error: error + "Ajout du like impossible" })
          );
      }
      ...
      if (stateLike == -1) {
        Sauce.updateOne(
          ...
          .catch((error) =>
            res
              .status(400)
              .json({ error: error + "Ajout du dislike impossible" })
          );
      }
        ...
        if (sauce.usersLiked.includes(idLikeUser)) {
          Sauce.updateOne(
            ...
            .catch((error) =>
              res
                .status(400)
                .json({ error: error + "Suppression du Like impossible" })
            );
        }
    ...
    .catch((error) =>
      res.status(500).json({ error: error + " erreur lors du like" })
    );
};
